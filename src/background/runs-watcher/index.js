...
async function checkPreviousRuns() {
    ...
    if ((runsItems.value()[1] != undefined && runsItems.value()[1].runEnd.date === null && runsItems.value()[1].toRemove.checkedByUser === false)) {
...
async function isSameRun(seed) {
    if (!inRun) inRun = true
    const runsItems = await runs
    return runsItems.filter(run => run.runEnd.date === null || run.runEnd.win === null).find(function(run, i) {
        if (currentRun.id) {
