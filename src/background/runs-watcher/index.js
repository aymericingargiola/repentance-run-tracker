...
function setInRun(status) {
    inRun = status
    if (win) syncApp(win,{trigger: "in run status", inRun: status})
}

async function checkPreviousRuns() {
    console.time("Check previous runs")
    const runsItems = await runs;
    if(previousRun && previousRun.runEnd && previousRun.runEnd.date === null && previousRun.runEnd.win === null && previousRun.gameState === currentGameState) {
        console.log("Direct unfinished previous run found, removing...", previousRun.id)
        syncApp(win,{trigger: "remove run", run: runsItems.value()[1].id})
        if(winTracker) syncApp(winTracker,{trigger: "remove run", run: previousRun.id})
        await runsItems.remove({id: previousRun.id}).write()
    }
    else if ((runsItems.value()[1] != undefined && runsItems.value()[1].runEnd.date === null && runsItems.value()[1].toRemove.checkedByUser === false)) {
        let deleted = false
        ...
        if (repentanceLogsArray[currentRunIndex - 2].includes("CPU time")) {
            ...
            syncApp(win,{trigger: "remove run", run: runsItems.value()[1].id})
            if(winTracker) syncApp(winTracker,{trigger: "remove run", run: runsItems.value()[1].id})
            //runs.splice(1, 1)
            await runsItems.remove({id: runsItems.value()[1].id}).write()
            //runsToRemove.push(runsItems.value()[1].id)
            deleted = true
        } else console.log("Run was not directly generated, check if same game state...")
        if (!deleted && runsItems.value()[1].gameState === currentGameState) {
            console.log("Current run was generated over an unfinished run on the same game state, ask user if he wants to remove it")
            runsItems.value()[1].toRemove.status = true
            sameGameStateAlreadyChecked = true
            syncApp(win,{trigger: "ask remove run", run: runsItems.value()[1]})
        ...
        if (!sameGameStateAlreadyChecked) {
            ...
            let unfinishedRuns = runsItems.filter(run => {
                if(run.id != runsItems.value()[0].id && run.runEnd.date === null && run.gameState === currentGameState && run.toRemove.checkedByUser === false) return run
            }).value()
            if (unfinishedRuns != undefined && unfinishedRuns.length > 0) {
                ...
                unfinishedRuns.forEach(run => {
                    runsItems.find({id: run.id}).get('toRemove').assign({status: true})
                })
...
async function isSameRun(seed, notRunEnd) {
    ...
    const check = runsItems.filter(run => (run.runEnd.date === null || run.runEnd.win === null) && run.gameState === currentGameState).find(function(run, i) {
        if (currentRun.id) {
            ...
            console.log(`compare run ${run.seed} with current ${seed}`)
            if (
                run.seed === seed
                ) {
...
async function updateOrCreateRun(params = {}) {
    ...
    if (sameRun) {
        ...
        if (runsToRemove.length > 0) {
            await runsItems.remove((run) =>  runsToRemove.includes(run.id)).write()
            runsToRemove = []
        }
        await runsItems.unshift(run).write()
...
function parseLogs(newLogs, logArray) {
    asyncForEach(newLogs, async (log) => {
...
setInterval(() => {
    ...
    if (watchingLogs && currentSameRun && !currentSameRun.runEnd.date) {
...
async function init() {
    ...
    if (seedsList[seedsList.length - 1] != undefined && !isShutdown) {
        ...
        setInRun(lastLogs.filter(v=>v.includes("Menu Game Init")).length < 1 && lastLogs.filter(v=>v.includes("Game Over")).length < 1 && lastLogs.filter(v=>v.includes("playing cutscene")).length < 1)
...
ipcMain.on('USER_REMOVE_RUN', async (event, payload) => {
...
ipcMain.on('USER_RESTORE_RUNS_FROM_TRASH', async (event, payload) => {
    console.log(`User wants to restore runs from trash : ${payload}`)
    await restoreRunsFromTrash(payload, runs, runsJsonPath, win, winTracker, trash, trashJsonPath)
})
