    ...
    <transition name="show">
        <div v-if="errors.length > 0" class="pop-up app-error">
            <div class="content">
                ...
                <p class="message">{{$t('appError.messagePart1')}} <a href="https://github.com/aymericingargiola/repentance-run-tracker/issues/new?assignees=aymericingargiola&labels=bug&template=bug_report.md&title=" target="_blank">{{$t('dictionary.here')}}</a> {{$t('appError.messagePart2')}} <br><br>{{$t('appError.messagePart3')}}</p>
                <ul class="error-area">
                ...
                <div class="actions">
                    <div class="btn generate">
                        <button @click="generateZipDump()">{{$t('appError.generateZip')}}</button>
                    </div>
                    <div class="btn restart">
                        <button @click="restartApp()">{{$t('strings.restartApp')}}</button>
                    </div>
                    <div class="btn close">
                        <button @click="closeApp()">{{$t('strings.closeApp')}}</button>
                    </div>
                </div>
